...
public void pintavalvsmci(){
	...
	for (i=0;i<Irrisoft.NUMVALVSMCI;i++){
		...
		if (i<9){
			...
			Irrisoft.window.valvsmci.getvalvmci(i).getImgasoc().setName("imglbl"+(i+1));
		}
		...
		if (i==0){
			...
			w=75;
			baj = baj +36;
			Irrisoft.window.valvsmci.getvalvmci(i).getImgasoc().setBounds(x+baj, y, w, z);
		}
...
public void interruptor (Valvula valv, JLabel img){
	...
	if (tipoplaca==1){
		try {
			conserie.conectaserial(Irrisoft.config.getMci(),tipoplaca);
		} catch (Exception e1) {
			//e1.printStackTrace();
		...
		try {
			conserie.conectaserial(Irrisoft.config.getMci2(),tipoplaca);
		} catch (Exception e1) {
			//e1.printStackTrace();
			Irrisoft.window.textArea.append("\nApertura de la valv "+valv+" cancelada.");
	...
	if (valv.isAbierta()){
			...
			//
			Irrisoft.window.valvsmci.getvalvmci(Integer.parseInt(valv.getCodelecvalv())-1).setAbierta(false);
			//
			//La quito de la lista de abiertas
			Irrisoft.window.quitarvalvabiertas(valv.getCodelecvalv());
			
			//paro el conteo de pulsos regando y lo paso a automáticooor
		...
		System.out.println("Está cerrada y la abro con la placa "+ tipoplaca);
			...
			//
			Irrisoft.window.valvsmci.getvalvmci(Integer.parseInt(valv.getCodelecvalv())-1).setAbierta(true);
			//
			//La meto en Valvsabiertas
			Irrisoft.window.hilocaudal.setValvsabiertas(Irrisoft.window.valvsabiertas());
			//Empiezo el conteo de pulsos caudalimetro
			Irrisoft.window.caudalregando(true);
			Irrisoft.window.hilocaudal.valv=valv;
