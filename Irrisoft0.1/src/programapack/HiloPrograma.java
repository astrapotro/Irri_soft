...
public class HiloPrograma implements Runnable{
				...
				try {
			 	...
			 	if (date2.after(date1)){
					...
					Irrisoft.window.frmIrrisoft.repaint();
				 	...
				 	//Duermo el hilo hasta que sea la h	System.out.println(("HOLA for dias"));ora 
				 	try {
						...
						if (delay>0){
							...
							System.out.println("Ya es la hora, y tengo que abrir durante "+prog.getDuracion());
						 	
						 	
							//llamo a abrirvalvulas
							if(valv<=28){
								accionvalv(prog,Irrisoft.config.getMci(),1);
							}else if (valv>28 && valv<1000){
								accionvalv(prog,Irrisoft.config.getMci2(),2);
							}else if(valv>1000 && valv <2000){
								accionvalv(prog,Irrisoft.config.getBt2(),3);
							}else if (valv >2000){
								accionvalv(prog,Irrisoft.config.getBt22(),4);
							}
						}
		...
		private void gestvalv(int tipo,boolean abierta){
			...
			if (tipo==1 || tipo==2){
				...
				if(tipo==3){
					codelecvalv = Integer.parseInt(prog.getCodelecvalv())-1001;
					if (abierta){
						//Actualizo estado de la valvula
					...
					codelecvalv = Integer.parseInt(prog.getCodelecvalv())-2001;
					...
					ListaValvBt22.getInstance().getvalvbt2(codelecvalv).setAbierta(true);
						...
						//Asocio una idtarea con la valvula
						ListaValvBt22.getInstance().getvalvbt2(codelecvalv).setTareaasoc(prog.getIdtareaprog());
					}
